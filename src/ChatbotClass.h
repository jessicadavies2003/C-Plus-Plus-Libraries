...
private:
	...
	StrStrDictionary variables;
	vector<string> varTags;
...
public:
	Chatbot(string botName = "Bot", string userName = "User", bool addDefaultPairs = true, bool addDefaultSubs = true) {
		variables.addKeyValue("username", userName);
		variables.addKeyValue("botname", botName);
		variables.addKeyValue("favcolour", "blue");

		for (string key : variables.getKeys()) {
			string newKey = "<" + key + ">";
			varTags.push_back(newKey);
		}
	...
	void setStrVariable(string varName, string value) {
		variables.removeKeyValue(varName);
		variables.addKeyValue(varName, value);
	}

	void talkWithBot() {
		cout << "Talking with " << grabVariable("botname") << ". Type 'exit' to leave chat." << endl;
		cout << "============================================" << endl;
		cout << endl;
